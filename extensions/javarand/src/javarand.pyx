...
cdef long long multiplier = 0x5DEECE66D
cdef long long addend = 0xB
cdef long long mask = (1 << 48) - 1

cdef double DOUBLE_UNIT = 1.0 / (1 << 53)

# IllegalArgumentException messages
cdef const char* BadBound = "bound must be positive"
cdef const char* BadRange = "bound must be greater than origin"
cdef const char* BadSize  = "size must be non-negative"
...
cdef class Random:
    ...
    def __init__(self, seed=None):
        ...
        if seed is None:
            seed = self.seedUniquifier() ^ nanoTime()
        self.seed = self.initialScramble(seed)

    cdef long long seedUniquifier(self):
        # L'Ecuyer, "Tables of Linear Congruential Generators of
        # Different Sizes and Good Lattice Structure", 1999
        self.__seedUniquifier *= 1181783497276652981l
        return self.__seedUniquifier
    ...
    #     while i < bytes_len:
    #         rnd = self.nextInt()
    #         n = np.min(bytes_len - i, 4)
    #         while n > 0:
    #             n -= 1
    #             bytes[i] = np.sign(rnd) * (rnd & 0xff)
    #             i += 1
    #             rnd >>= 8
    ...
    cpdef long long nextLong(self):
        ...
        return (<long long>(self.nextBits(32)) << 32) + self.nextBits(32)
    ...
    cpdef float nextFloat(self):
        return self.nextBits(24) / (<float>(1 << 24))
    ...
    cpdef double nextGaussian(self):
        ...
        cdef double multiplier, v1, v2, s
        if self.haveNextNextGaussian:
            ...
            v1 = v2 = s = 0
            while s >= 1 or s == 0:
                v1 = 2 * self.nextDouble() - 1 # between -1 and 1
                v2 = 2 * self.nextDouble() - 1 # between -1 and 1
                s = v1 * v1 + v2 * v2
